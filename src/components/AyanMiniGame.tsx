...
const roasts = ["Nice try being the GOAT, Ayan!", "You're more like a baby goat!", "GOAT? More like GHOST... disappearing!", "Ayan thought he was special!", "Caught you slacking!", "Where's your crown now?", "Not so fast, 'GOAT'!", "Ayan.exe has stopped working!", "Legend says Ayan is still running!", "Even a turtle is faster!", "Is that all you got, Ayan?", "The GOAT title has been revoked!", "Ayan needs more practice!", "Too slow, 'champion'!", "Your throne is crumbling!"];
export const AyanMiniGame = ({
  onExit
}: AyanMiniGameProps) => {
  const [difficulty, setDifficulty] = useState<Difficulty | null>(null);
  const [ayanPosition, setAyanPosition] = useState({
    x: 50,
    y: 50
  });
  ...
  if (!difficulty) {
    return <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-hero animate-fade-in p-4">
        {/* Floating particles */}
        ...
        <div className="text-center relative z-10">
          <h1 className="text-5xl md:text-6xl font-bold bg-gradient-primary bg-clip-text text-transparent mb-4 animate-glow-pulse">Conquer SOMEONE!</h1>
          <p className="text-xl text-muted-foreground mb-8">Select your difficulty</p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-xl mx-auto">
            <button onClick={() => startGame("easy")} className="bg-card/80 backdrop-blur-sm border-2 border-green-500/50 hover:border-green-500 p-6 rounded-xl transition-all hover:scale-105 hover:shadow-lg">
            ...
            <button onClick={() => startGame("medium")} className="bg-card/80 backdrop-blur-sm border-2 border-yellow-500/50 hover:border-yellow-500 p-6 rounded-xl transition-all hover:scale-105 hover:shadow-lg">
            ...
            <button onClick={() => startGame("hard")} className="bg-card/80 backdrop-blur-sm border-2 border-orange-500/50 hover:border-orange-500 p-6 rounded-xl transition-all hover:scale-105 hover:shadow-lg">
            ...
            <button onClick={() => startGame("impossible")} className="bg-card/80 backdrop-blur-sm border-2 border-destructive/50 hover:border-destructive p-6 rounded-xl transition-all hover:scale-105 hover:shadow-lg">
  ...
  return <div className={`min-h-screen flex flex-col items-center justify-center bg-gradient-hero animate-fade-in p-4 transition-all ${screenShake ? 'animate-pulse' : ''}`}>
      ...
      <div className="text-center mb-4 relative z-10">
        ...
        <div className="flex gap-4 md:gap-8 justify-center text-lg md:text-xl flex-wrap">
          ...
          {comboCount > 1 && <div className="bg-primary/20 backdrop-blur-sm px-4 py-2 rounded-lg border border-primary animate-bounce">
      ...
      {showRoast && <div className="fixed top-1/4 left-1/2 -translate-x-1/2 z-50 animate-fade-in pointer-events-none">
      ...
      {/* Game area */}
      <div className={`relative w-full max-w-2xl h-80 bg-card/80 backdrop-blur-sm border-2 border-primary/30 rounded-xl overflow-hidden shadow-card transition-transform ${screenShake ? 'scale-[1.02]' : ''}`}>
        {/* Click effects */}
        {clickEffects.map(effect => <div key={effect.id} className="absolute pointer-events-none" style={{
        left: effect.x,
        top: effect.y
      }}>
            <div className="w-20 h-20 -translate-x-1/2 -translate-y-1/2 rounded-full bg-primary/50 animate-ping" />
            <div className="absolute inset-0 w-12 h-12 -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent/70 animate-ping" style={{
          animationDelay: '0.1s'
        }} />
          </div>)}
        ...
        <div className="absolute top-0 left-0 right-0 h-2 bg-muted">
          ...
          width: `${score / settings.clicksNeeded * 100}%`
        }} />
        </div>

        {!gameOver && <button onClick={handleClick} className="absolute transition-all duration-150 hover:scale-150 cursor-pointer select-none group" style={{
        left: `${ayanPosition.x}%`,
        top: `${ayanPosition.y}%`,
        ...
        {gameOver && <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/95 backdrop-blur-sm">
            {conquered ? <>
      ...
      <div className="flex gap-4 mt-6 relative z-10 flex-wrap justify-center">
        {gameOver && <Button onClick={resetGame} className="bg-primary hover:bg-primary-glow shadow-intense hover:scale-105 transition-all">
            Play Again
          </Button>}
